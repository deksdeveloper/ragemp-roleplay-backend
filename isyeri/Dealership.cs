...
public class Dealership : Script
{
    ...
    public static void PreviewVehicleDealership(Player player, string vehname)
    {
        ...
        player.SetData("galeriDimension", dimen);
    ...
    public static void DealershipExitMenu(Player player)
    {
        ...
        player.SetData("galeriDimension", -1);
    }
    ...
    public static void BuyVehicle(Player Client, string model)
    {
        ...
        if (index == -1)
        {
            Notify.Send(Client, "error", PlayerVehicle.MAX_PLAYER_VEHICLES + " araçtan fazlasını satın alamazsınız.", 5000);
            return;
        }
        else
        {
            ...
            {
                Notify.Send(Client, "error", "Yeterli miktarda paranız yok.", 5000);
                return;
            }
            ...
            Main.SendSuccessMessage(Client, $"Başarıyla {model} model araç satın aldınız!");
    ...
    public void AddVehicleToBusiness(Player Client, string name, int stock, int price)
    {
        ...
        if (business_id == -1)
        {
            Main.DisplayErrorMessage(Client, "error", "Bu işyerinin sahibi değilsiniz!");
            Client.TriggerEvent("Destroy_Character_Menu");
            return;
        }
        ...
        using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
        {
            ...
            if (index == -1)
            {
                Main.SendErrorMessage(Client, "Bir hata oluştu: ~y~" + dealership_vehicle_name + "~w~. ~r~[KOD #000010]");
                Client.TriggerEvent("Destroy_Character_Menu");
                return;
            }
            ...
            if (i == 0)
            {
                ...
                {
                    Notify.Send(Client, "error", "İşletme kasasında yeterli paranız yok.", 5000);
                    return;
                }
                ...
                Main.SendSuccessMessage(Client, "Başarıyla " + dealership_vehicle_stock + " adet araç satın aldınız. " + dealership_vehicle_name + " $~g~" + dealership_vehicle_price.ToString("N0") + ". İşletme menüsünden sipariş verebilirsiniz.");
            ...
            else
            {
                using (MySqlDataReader reader = query.ExecuteReader())
                {
                    ...
                    if (BusinessManage.business_list[business_id].business_Safe < dealership_vehicle_price)
                    {
                        Notify.Send(Client, "error", "Yeterli paranız yok.", 5000);
                        return;
                    }
    ...
    public void SaveVehicleToBusiness(Player Client, string name, int price, int new_state)
    {
        ...
        {
            Client.SendNotification("~r~Hata:~w~ Yanlış değer.");
            return;
        }
        ...
        Notify.Send(Client, "error",name + "model araç güncellendi. Yeni fiyat: $" + price.ToString("N0") + "~n~~n~~w~Depo: " + response + "", 5000);
        ShowDealershipManage(Client);
    }
}
