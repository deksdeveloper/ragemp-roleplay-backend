...
class carshop : Script
{
    ...
    public carshop()
    {
        NAPI.TextLabel.CreateTextLabel(Main.LabelColor + "(( Galeri ))~n~" + Main.LabelCommandColor + "« Y »", new Vector3(-57.31, -1096.82, 26.42), 16, 0.600f, 0, new Color(0, 122, 255, 150));

        NAPI.TextLabel.CreateTextLabel(Main.LabelColor + "Aracinizi %60 fiyata satmak istiyor musunuz?~n~" + Main.LabelCommandColor + "« /aracisat »", new Vector3(-8.23, -1082.93, 26.85), 12, 0.3500f, 4, new Color(0, 122, 255, 150));


        Entity temp_blip;
        temp_blip = NAPI.Blip.CreateBlip(new Vector3(-57.31, -1096.82, 26.42));
        NAPI.Blip.SetBlipName(temp_blip, "Galeri");
        ...
        NAPI.Task.Run(() =>
        {
            ...
            NAPI.Task.Run(() =>
            {
                ...
                if (NAPI.Player.IsPlayerConnected(player))
                {
                    player.Dimension = 0;
                    player.Position = new Vector3(-57.31, -1096.82, 26.42);
                    Main.SendErrorMessage(player, "Test sürüşünün süresi doldu!");
                }
    ...
    public void VehBuy(Player player, string vehname, int color)
    {
        try
        {
            ...
            if (index == -1)
            {
                Notify.Send(player, "error", PlayerVehicle.MAX_PLAYER_VEHICLES + " araçtan fazlasını satın alamazsınız.", 5000);
                return;
            }
            ...
            if (Vehicles.TryGetValue(vehname, out vehicleData))
            {
                ...
                int bankMoney = Main.GetPlayerBank(player);
                int playerMoney = Main.GetPlayerMoney(player);
                if (playerMoney >= vehiclePrice)
                {
                    ...
                    Main.GivePlayerMoney(player, -vehiclePrice);
                    ...
                    NAPI.Task.Run(() =>
                    {
                        if (NAPI.Player.IsPlayerConnected(player))
                        {
                            ...
                            Main.SendSuccessMessage(player, "Başarıyla " + vehname + " model aracı satın aldınız, aracı garajdan çıkartabilirsiniz!");
                            player.Vehicle.Delete();
                            player.Dimension = 0;
                            player.Position = new Vector3(-57.31, -1096.82, 26.42);
                            player.TriggerEvent("freezeEx", false);
                ...
                else if (bankMoney >= vehiclePrice)
                {
                    uint vehicleHash = NAPI.Util.GetHashKey(vehname);
                    Vector3 vehiclePosition = new Vector3(0, 0, 0);
                    PlayerVehicle.CreatePlayerVehicle(player, index, vehname, -46, -1081, 26, 0, 0, 70, color, color, 0);
                ...
                {
                    Main.DisplayErrorMessage(player, "error", "Banka hesabınızda veya üzerinizde yeterli miktarda paranız yok.");
                }
            }
            else
            {
                Notify.Send(player, "error", vehname + " model araç galeride mevcut değil!", 5000);
            }
        }
        catch (Exception ex)
        {
            NAPI.Util.ConsoleOutput("[HATA] " + ex.Message);
        }
    ...
    [Command("aracisat")]
    public void aracisatCMD(Player player)
    {
        try
        {
            if (player.GetData<dynamic>("status") == true)
            {
                if (Main.IsInRangeOfPoint(player.Position, new Vector3(-8.23, -1082.93, 26.05), 5.0f))
                {
                    ...
                    {
                        Notify.Send(player, "error", $"Sürücü koltuğunda olmalısınız.", 3000);
                        return;
                    }
                    ...
                    if (player.Vehicle.GetData<dynamic>("Mashin_Owner") == AccountManage.GetPlayerSQLID(player))
                    {
                        ...
                        if (!vehicleExists)
                        {
                            Notify.Send(player, "error", $"Araç veritabanında bulunmuyor.", 3000);
                            return;
                        }
                        ...
                        if (price > 0)
                        {
                            ...
                            Main.SendSuccessMessage(player, "Başarıyla aracınızı $" + sellPrice.ToString("N0") + " karşılığında sattınız!");
                            veh.Delete();
                        }
                        else
                        {
