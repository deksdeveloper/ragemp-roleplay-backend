...
class InteractMenu_New : Script
{
    ...
    public static void pSelected(Player Client, params object[] args)
    {
        ...
        {
            Main.DisplayErrorMessage(Client, "error", "Etkileşime geçmek istediğiniz kişiye yakın değilsiniz!");
            return;
        }
        ...
        switch (action)
        {
            case "give money":
                ...
                InteractMenu.User_Input(Client, "responser_money_user", "" + target.Name + " adlı oyuncuya para veriyorsunuz", "0", "", "number");
                return;
            ...
            case "cufuncuff":
                {
                    ...
                    {
                        NAPI.Notification.SendNotificationToPlayer(Client, "~r~Hata:~w~ Bu işlemi gerçekleştirebilmek için yetkiniz yok!");
                        return;
                    }
                    if (target.GetData<dynamic>("playerCuffed") == 0)
                    {
                        if (target != null)
                        {
                            if (Main.IsInRangeOfPoint(target.Position, Client.Position, 3) && target != Client)
                            {
                                ...
                                {
                                    NAPI.Notification.SendNotificationToPlayer(Client, "~r~Hata:~w~ Oyuncunun zaten kelepçesi var.");
                                    return;
                                }

                                if (target.GetData<dynamic>("handsup") == false)
                                {
                                    NAPI.Notification.SendNotificationToPlayer(Client, "~r~Hata:~w~ Oyuncu ellerini kaldırmadı.");
                                    return;
                                }

                                if (target.IsInVehicle)
                                {
                                    NAPI.Notification.SendNotificationToPlayer(Client, "~r~Hata:~w~ Oyuncu araçta.");
                                    return;
                                }

                                target.SetData<dynamic>("handsup", false);
                                NAPI.Notification.SendNotificationToPlayer(Client, "~y~BİLGİ:~w~ ~b~" + AccountManage.GetCharacterName(target) + "~w~ adlı oyuncuya kelepçe taktınız.");
                                NAPI.Notification.SendNotificationToPlayer(target, "~y~BİLGİ:~w~ ~b~" + AccountManage.GetCharacterName(Client) + "~w~ size kelepçe taktı.");
            ...
            case "showpas":
                {
                    ...
                    Main.SendCustomChatMessasge(target, $"<font color='#66ED00'>//----------------------------------------------------------------\\");
                    Main.SendCustomChatMessasge(target, $"<font color='#ffffff'>» Ad - Soyad: <font color='#C2A2DA'>{AccountManage.GetCharacterName(Client)}");
                    Main.SendCustomChatMessasge(target, $"<font color='#ffffff'>» Yaş: <font color='#C2A2DA'>{Client.GetData<dynamic>("character_age")}");
                    Main.SendCustomChatMessasge(target, $"<font color='#ffffff'>» Cinsiyet: <font color='#C2A2DA'>{Gender}");
                    if(fac != "Yok") Main.SendCustomChatMessasge(target, $"<font color='#ffffff'>» Birlik: <font color='#C2A2DA'>{fac}");
                    Main.SendCustomChatMessasge(target, $"<font color='#66ED00'>//----------------------------------------------------------------\\");
                    return;
                }
    ...
    public void toolselected(Player Client, params object[] args)
    {
        ...
        {
            Main.DisplayErrorMessage(Client, "error", "Predaleko ste!");
            return;
        }
        var action = args[1].ToString();
        switch (action)
            ...
            case "carrep":
                {
                                    ...
                                    if (NAPI.Entity.DoesEntityExist(veh))
                                    {
                                        ...
                                        repairTimer = TimerEx.SetTimer(() =>
                                        {
                                            ...
                                            if (time == 0)
                                            {
                                                ...
                                                Client.StopAnimation();
                                                Main.DisplayErrorMessage(Client, "info", "Vozilo je uspesno popravljeno");
                                            }
                                        }, 1000, 0);
                    ...
                    {
                        Main.DisplayErrorMessage(Client, "error", "Nemate alat za popravku!");
                    }
                    return;
                }
            case "refuel":
                    ...
                    if (NAPI.Entity.DoesEntityExist(veh))
                    {
                        ...
                        if (!Client.IsInVehicle)
                        {
                            if (NAPI.Entity.DoesEntityExist(veh))
                            {
                                ...
                                Main.DisplayErrorMessage(Client, "info", "Natocili ste 20L goriva u Vase vozilo");
...
//cclose
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            Main.DisplayErrorMessage(Client, "error", "Ne mozete zapleniti ovo vozilo.");
                            return;
                        }
                    }
                    connection.Close();
            ...
            switch (index)
            {
                        ...
                        for (int index2 = 0; index2 < PlayerVehicle.MAX_PLAYER_VEHICLES; index2++)
                        {
                            if (PlayerVehicle.vehicle_data[playerid].state[index2] == 1 && PlayerVehicle.vehicle_data[playerid].handle[index2].Exists && veh == PlayerVehicle.vehicle_data[playerid].handle[index2])
                            {
                                if (VehicleStreaming.GetLockState(veh) == false)
                                {
                                    ...
                                    Main.DisplayErrorMessage(Client, "success",  Translation.head_notification_2);
                                }
                                else
                                {
                                    VehicleStreaming.SetLockStatus(veh, false);
                                    Main.DisplayErrorMessage(Client, "success",  Translation.head_notification_1);
                                }
                                return;
                            }
                        }
                ...
                case 1:
                    if (!veh.HasData("Door_4") || veh.GetData<bool>("Door_4") == false)
                    {
                        ...
                        {
                            Main.DisplayErrorMessage(Client, "error", "Vozilo je zakljucano!");
                            return;
                        }
                        Main.EmoteMessage(Client, "otvara haubu.");
                    ...
                    {
                        Main.EmoteMessage(Client, "zatvara haubu.");
                ...
                case 2:
                    ...
                    {
                        Main.EmoteMessage(Client, "otvara gepek.");
                    ...
                    {
                        Main.EmoteMessage(Client, "zatvara gepek.");
                ...
                case 3:
                    ...
                    {
                        Main.EmoteMessage(Client, "otvara vrata.");
                    ...
                    {
                        Main.EmoteMessage(Client, "zatvara vrata.");
                ...
                case 7:
                    NAPI.Task.Run(() =>
                    {
                        ...
                        if (Client.IsInVehicle && Client.VehicleSeat == (int)VehicleSeat.Driver)
                        {
                            ...
                            {
                                Main.DisplaySuccess(Client, "info",  Translation.head_notification_6);
                                return;
                            }
                            ...
                            if (NAPI.Data.HasEntityData(Client.Vehicle, "vehicle_colision"))
                            {
                                ...
                                Main.DisplaySuccess(Client, "info",  Translation.head_notification_7);
                                return;
                            }
                            ...
                            if (Main.GetVehicleFuel(Client.Vehicle) <= 2)
                            {
                                ...
                                Main.DisplaySuccess(Client, "info",  Translation.head_notification_8);
                                return;
                            }
                            if (Client.Vehicle.HasData("IsInHighEnd") && Client.Vehicle.GetData<dynamic>("IsInHighEnd") == true)
                            {

                                Main.DisplayErrorMessage(Client, "error", "Nemate kljuc");
                                return;
                            }
                            ...
                            if (VehicleStreaming.GetEngineState(Client.Vehicle) == false)
                            {
                                ...
                                Main.DisplayErrorMessage(Client, "success",  Translation.head_notification_9);
                            }
                            else
                            {
    ...
    public static void vehicleSelected(Player Client, params object[] arguments)
    {
        try
        {
            ...
            {
                Main.DisplayErrorMessage(Client, "error", "Nema vozila pored Vas!");
                return;
            }
            switch (index)
            {
                        ...
                        if (pl.GetData<dynamic>("status") == true)
                        {
                            for (int index = 0; index < PlayerVehicle.MAX_PLAYER_VEHICLES; index++)
                            {
                                if (PlayerVehicle.vehicle_data[Main.getIdFromClient(pl)].state[index] == 1 && PlayerVehicle.vehicle_data[Main.getIdFromClient(pl)].handle[index].Exists && PlayerVehicle.vehicle_data[Main.getIdFromClient(pl)].handle[index] == veh2)
                                {
                                    ...
                                    Main.DisplayErrorMessage(Client, "info", "Izdali ste kaznu u iznosu od ~g~$" + fine.ToString("N0") + "~w~ za vozilo " + PlayerVehicle.vehicle_data[Main.getIdFromClient(pl)].handle[index].DisplayName + " ,tablice: ~b~" + PlayerVehicle.vehicle_data[Main.getIdFromClient(pl)].plate[index] + "~w~.");
    ...
    public void KeyPressO(Player Client)
    {
        if (AccountManage.GetPlayerConnected(Client))
        {
            ...
            {
                Main.DisplayErrorMessage(Client, "error", "Balık tutarken bu işlemi yapamazsınız!");
                return;
            }
        }
    }
    ...
    public static void ShowPlayerInteractMenu(Player Client)
    {
        ...
        if (Client.GetData<dynamic>("character_backpack") == 1 || Client.GetData<dynamic>("character_backpack") == 2)
            ...
            if (Client.IsInVehicle)
            {
                ...
                {
                    if (Client.Vehicle.EngineStatus == false) { Main.DisplayErrorMessage(Client, "error", "Morate ugasiti motor vozila"); }
    ...
    public static void Responder_Menu(Player Client, int selectedIndex, String objectName, String valueList)
    {
            ...
            if (Client.HasData("Gloves_Box") && Client.GetData<dynamic>("Gloves_Box") == true)
            {
                ...
                for (int i = 0; i < Inventory.MAX_INVENTORY_ITENS; i++)
                {
                    if (Inventory.player_inventory[Client.Value].type[i] == 43 && Inventory.player_inventory[Client.Value].inuse[i] == 1)
                    {
                        ...
                        Main.DisplayErrorMessage(Client, "success", "skinuli ste rukavice.");
                        return;
                    }
                }
            }
        ...
        {
            Main.DisplayErrorMessage(Client, "error", "Bacili ste ranac.");
        }
        ...
        else if (objectName == Translation.INTERACT_PLAYER_MENU_35)
        {
            ...
            {
                Main.SendErrorMessage(Client, "Yaralı değilsiniz.");
                return;
            }
            if (Client.GetSharedData<dynamic>("InjuredTime") != 0)
            {
                Main.SendErrorMessage(Client, "Yaralı süreniz sona ermemiş.");
                return;
            }

            Client.SetSharedData("Injured", 0);
            NAPI.Player.SetPlayerHealth(Client, 40);
            ...
            Main.SendSuccessMessage(Client, "Başarıyla yerden kalktın!");
            return;
        }
        ...
        else if (objectName == Translation.INTERACT_PLAYER_MENU_11)
        {
            ...
            else
            {
                Client.SetData<dynamic>("group_invite", -1);
                Client.SetData<dynamic>("group_inviteBy", -1);
                Main.DisplayErrorMessage(Client, "info", "Otkazali ste ponudu pridruzivanja organizaciji.");
        ...
        else if (objectName == Translation.INTERACT_PLAYER_MENU_25)
        {
            ...
            if (Client.IsInVehicle && Client.VehicleSeat == (int)VehicleSeat.Driver && NAPI.Entity.GetEntityModel(Client.Vehicle) == (uint)NAPI.Util.VehicleNameToModel("Taxi"))
            {
                if (!Client.Vehicle.HasData("TransportDuty"))
                {
                    ...
                    {
                        Main.DisplayErrorMessage(Client, "error", "Cena voznje ne moze biti manja od $1 ili veca od $200.");
                        return;
                    }
        ...
        /*   else if (objectName == Translation.INTERACT_PLAYER_MENU_36)
           {
               ...
               if (index == 0)
               {
                   Main.DisplayErrorMessage(Client, "error", "Вы не во фракции...");
                   ShowPlayerInteractMenu(Client);
                   return;
               }
        ...
        else if (objectName == Translation.INTERACT_PLAYER_MENU_38)
        {
            if (AccountManage.GetPlayerJob(Client) != 0)
            {
                ...
                Main.DisplayErrorMessage(Client, "info", "Nemate posao");
    ...
    public static void SelectMenuResponse(Player Client, String callbackId, int selectedIndex, String objectName, String valueList)
    {
        ...
        else if (callbackId == "SEARCH_WHOLE_INVENTORY")
        {
            ...
            try
            {
                ...
                Main.EmoteMessage(Client, "kupuje " + item + ".");
        ...
        else if (callbackId == "TARGET_MENU_REMOVE_ITEM")
        {
            ...
            Inventory.RemoveItem(target, item, amount);
            Inventory.GiveItemToInventoryFromName(Client, item, amount);
            Main.DisplayErrorMessage(Client, "info", "Oduzeli ste " + item + " od " + UsefullyRP.GetPlayerNameToTarget(target, Client) + ".");
            Main.DisplayErrorMessage(target, "info", "Sluzbenik " + UsefullyRP.GetPlayerNameToTarget(Client, target) + " vam je oduzeo " + item + ".");
            ...
            if (callbackId == "NEW_MENU_RESPONSE")
            {
                ...
                else if (objectName == Translation.INTERACT_PLAYER_MENU_32)
                {
                    ...
                    if (Client.GetData<dynamic>("hats_enable") == true) // false
                    {
                        ...
                        Main.DisplayErrorMessage(Client, "info", "Skinuli ste kapu");
                    ...
                    {
                        Main.DisplayErrorMessage(Client, "info", "Stavili ste kapu");
                ...
                else if (objectName == Translation.INTERACT_PLAYER_MENU_33)
                {
                    ...
                    {
                        Main.DisplayErrorMessage(Client, "info", "Skinuli ste naocare");
                    ...
                    {
                        Main.DisplayErrorMessage(Client, "info", "Stavili ste naocare");
    ...
    public static void SendNotificationError(Player Client, string errorMessage)
    {
        ...
        Main.SendErrorMessage(Client, errorMessage);
    }
    public static void SendNotificationInfo(Player Client, string infoMessage)
    {
        Main.SendInfoMessage(Client, infoMessage);
    }
