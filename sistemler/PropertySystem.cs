...
class PropertySystem : Script
{
        ...
        using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
        {
            ...
            using (MySqlDataReader reader = query.ExecuteReader())
            {
                ...
                while (reader.Read())
                {
                    ...
                    {
                        entrace_data[index].label = NAPI.TextLabel.CreateTextLabel("~y~ Bu mülk \n ~g~ $ karşılığında satılık" + entrace_data[index].price + "\n ~y~/buyproperty", entrace_data[index].exterior, 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[index].exterior_dimension);
                    }
                    else
                    {
                        if (entrace_data[index].description != "32")
                        {
                            entrace_data[index].label = NAPI.TextLabel.CreateTextLabel("~y~" + entrace_data[index].name + "~w~~n~~n~" + entrace_data[index].description, entrace_data[index].exterior, 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[index].exterior_dimension);
                        }
                        else
                        {
                            entrace_data[index].label = NAPI.TextLabel.CreateTextLabel("~y~" + entrace_data[index].name + "~w~~n~~n~", entrace_data[index].exterior, 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[index].exterior_dimension);
                        }
                    }

                    entrace_data[index].label_interior = NAPI.TextLabel.CreateTextLabel("- ~y~Exit~w~ -", entrace_data[index].interior, 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[index].exterior_dimension);
                    ...
                    /*if (float.Parse(reader.GetString("sellpoint").Split(",")[0]) != 0)
                    {
                        ...
                        entrace_data[index].sellpoint_label = NAPI.TextLabel.CreateTextLabel("~w~[ ~y~E ~w~]", new Vector3(float.Parse(reader.GetString("sellpoint").Split(",")[0]), float.Parse(reader.GetString("sellpoint").Split(",")[1]), float.Parse(reader.GetString("sellpoint").Split(",")[2])), 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[index].interior_dimension);
                    ...
                    {
                        entrace_data[index].manage_label = NAPI.TextLabel.CreateTextLabel("Mülk~n~~n~~y~Tip: ~g~" + entrace_data[index].business_type.ToString() + "~n~~y~/buyproperties ~n~~y~Price: ~g~$" + entrace_data[index].price, manage, 5, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[index].interior_dimension);
                    }
                    else
                    {
                        entrace_data[index].manage_label = NAPI.TextLabel.CreateTextLabel("", manage, 5, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[index].interior_dimension);
                    }
    ...
    public static void PressKeyY(Player Client)
    {
        if (Client.HasData("PB_index"))
        {
            ...
            if (Main.IsInRangeOfPoint(Client.Position, entrace_data[propid].exterior, 2.0f) && Client.Dimension == entrace_data[propid].exterior_dimension)
            {
                ...
                {
                    Main.DisplayErrorMessage(Client, "info", "Dar Ghofl Ast.");
                    return;
                }
    ...
    public void PropertySearchID(Player player, int id)
    {
        ...
        {
            Main.DisplayErrorMessage(player, "info", "Geçersiz ID!");
            return;
        }
        Main.SendCustomChatMessasge(player, "~y~[~g~Property~y~] ~y~Sahip: ~w~" + AccountManage.GetPlayerNameFromSQL(entrace_data[id].owner_id) + " ~y~Pos Girin: ~w~" + entrace_data[id].exterior.X + "," + entrace_data[id].exterior.Y + "," + entrace_data[id].exterior.Z);
    }
    ...
    public void AdminEditPrvateProperty(Player client, int PBID, string type, string value = "0")
    {
        if (AccountManage.GetPlayerConnected(client))
        {
            ...
            {
                Main.DisplayErrorMessage(client, "info", "Geçersiz ID!");
                return;
            }
                    ...
                    case "name":
                        {
                            ...
                            {
                                Main.DisplayErrorMessage(client, "info", "İsim 3 ile 31 karakter arasında olmalıdır.");
                                return;
                            }
                    ...
                    case "price":
                        {
                            ...
                            else
                            {
                                ...
                                entrace_data[PBID].manage_label.Text = "Mülk~n~~n~~y~Tip: ~g~" + entrace_data[PBID].business_type.ToString() + "~n~~y~/buyproperties ~n~~y~Fiyat: ~g~$" + entrace_data[PBID].price;
                                SaveEntrace(PBID);
                            }
                    ...
                    case "management":
                        {
                            ...
                            else
                            {
                                ...
                                entrace_data[PBID].manage_colshape.SetData("PB_index", PBID);
                                entrace_data[PBID].manage_label = NAPI.TextLabel.CreateTextLabel("Kullanmak için [ ~y~E ~w~] ~n~~w~ mülk yönetimi", client.Position, 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[PBID].interior_dimension);
                            }
                            SaveEntrace(PBID);
                    ...
                    case "sellpoint":
                        {
                            ...
                            {
                                entrace_data[PBID].sellpoint_label = NAPI.TextLabel.CreateTextLabel("[ ~y~E ~w~]", client.Position, 5, 0.6f, 4, new Color(0, 122, 255, 150));
                    ...
                    case "description":
                        {
                            ...
                            {
                                Main.DisplayErrorMessage(client, "info", "Açıklama 2 ile 127 karakter arasında olmalıdır");
                                return;
                            }
    ...
    public void BuyProperties(Player player)
    {
        ...
        if (entrace_data.ElementAtOrDefault(PBID) == null)
        {
            Main.DisplayErrorMessage(player, "error", "İş yerinin yankınında değilsiniz!");
            return;
        }
        if (Main.GetPlayerBank(player) < entrace_data[PBID].price)
        {
            Main.DisplayErrorMessage(player, "error", "Yeterli paranız yok!");
            return;
        }
        ...
        entrace_data[PBID].manage_label.Text = "İş yeri yönetimi için [ ~y~E ~w~] tuşunu kullanın";
        entrace_data[PBID].owner_id = AccountManage.GetPlayerSQLID(player);
        entrace_data[PBID].isforsell = 0;
        Main.DisplayErrorMessage(player, "success", "İş yerini satın aldınız!");
        SaveEntrace(PBID);
    }
    ...
    public void sellproperty(Player player, int price)
    {
        ...
        if (entrace_data.ElementAtOrDefault(PBID) == null)
        {
            Main.DisplayErrorMessage(player, "error", "İş yeriniz yok!");
            return;
        }
        if (entrace_data[PBID].owner_id != AccountManage.GetPlayerSQLID(player))
        {
            Main.DisplayErrorMessage(player, "error", "Bu iş yerinin sahibi değilsiniz!");
            return;
        }
        if (price == 0)
        {
            Main.DisplayErrorMessage(player, "success", "Nekretnik satıldı!");
            entrace_data[PBID].manage_label.Text = "İş yeri yönetimi için [ ~y~E ~w~] tuşunu kullanın";
            entrace_data[PBID].isforsell = 0;
            SaveEntrace(PBID);
            return;
        ...
        entrace_data[PBID].manage_label.Text = "İş yeri~n~~n~~y~Tür: ~g~" + entrace_data[PBID].business_type.ToString() + "~n~~y~/buyproperties ~n~~y~Fiyat: ~g~$" + entrace_data[PBID].price;
        entrace_data[PBID].isforsell = 1;
        entrace_data[PBID].price = price;
        SaveEntrace(PBID);
        Main.DisplayErrorMessage(player, "success", "İş yeri satışa çıkarıldı");
        Main.SendMessageToPlayer(player, "[~y~İş yeri~w~] Tebrikler, İş yerinizi ~g~" + price.ToString("N0") + "$~w~ karşılığında sattınız.");
    }
    ...
    public void EditPrivateBusiness(Player client, string type, string value = "0")
    {
        if (AccountManage.GetPlayerConnected(client))
        {
            ...
            if (entrace_data.ElementAtOrDefault(PBID) == null)
            {
                Main.DisplayErrorMessage(client, "info", "İş yerine sahip değilsiniz.");
                return;
            }
                    ...
                    case "name":
                        {
                            if (client.HasData("PB_index"))
                            {
                                if (value.Length < 2 || value.Length > 31)
                                {
                                    Main.DisplayErrorMessage(client, "info", "İsim, 2 ile 31 karakter arasında olmalıdır!");
                                    return;
                                }
                                entrace_data[PBID].name = value;
                                entrace_data[PBID].label.Text = "~y~" + entrace_data[PBID].name + "~w~~n~~n~" + entrace_data[PBID].description;
    ...
    public static void property_deposit_fund(Player player, int value)
    {
        ...
        if (entrace_data.ElementAtOrDefault(PBID) == null)
        {
            Main.DisplayErrorMessage(player, "info", "İş yerine sahip değilsiniz.");
            return;
        }
        if (entrace_data[PBID].owner_id != AccountManage.GetPlayerSQLID(player))
        {
        ...
        {
            Main.SendCustomChatMessasge(player, "~w~[~g~Mülk~w~] Yeterli paranız yok!");
            return;
        }
    ...
    public static void property_withdraw_fund(Player player, int value)
    {
        ...
        if (entrace_data.ElementAtOrDefault(PBID) == null)
        {
            Main.DisplayErrorMessage(player, "info", "İş yeri sahibi değilsiniz.");
            return;
        }
        if (entrace_data[PBID].owner_id != AccountManage.GetPlayerSQLID(player))
        {
        ...
        {
            Main.SendCustomChatMessasge(player, "~w~[~g~Mülk~w~] İş yerinde yeterli para yok!");
            return;
        }
        entrace_data[PBID].safemoney -= value;
        Main.GivePlayerMoney(player, value);
    ...
    public static void property_change_name(Player player, string name)
    {
        ...
        if (entrace_data.ElementAtOrDefault(PBID) == null)
        {
            Main.DisplayErrorMessage(player, "info", "İş yeri sahip değilsiniz.");
            return;
        }
        if (entrace_data[PBID].owner_id != AccountManage.GetPlayerSQLID(player))
        {
        ...
        {
            Main.SendCustomChatMessasge(player, "~w~[~g~Mülk~w~] Biznis adı en az 4 ve en fazla 50 karakter olmalıdır.");
            return;
        }
    ...
    public static void Buypropertie_Stock(Player player, int type, string picture, string name, int stock, int price)
    {
        ...
        if (entrace_data.ElementAtOrDefault(PBID) == null)
        {
            Main.DisplayErrorMessage(player, "info", "İş yeriniz yok.");
            return;
        }
        if (entrace_data[PBID].owner_id != AccountManage.GetPlayerSQLID(player))
        {
        ...
        {
            Main.SendCustomChatMessasge(player, "~w~[~g~Mülk~w~] İş yerinizin kasasında yeterli para yok.");
            return;
        }
        int index = 0;
        foreach (var item in entrace_data[PBID].Products)
    ...
    public static void ShowPropertyManagement(Player Client)
    {
        ...
        if (entrace_data.ElementAtOrDefault(PBID) == null)
        {
            Main.DisplayErrorMessage(Client, "info", "İş yeriniz yok.");
            return;
        }
        if (entrace_data[PBID].owner_id != AccountManage.GetPlayerSQLID(Client))
        {
        ...
        Client.TriggerEvent("Display_property_Manage", entrace_data[PBID].name, "İş Yeri Yönetimi", entrace_data[PBID].safemoney, NAPI.Util.ToJson(product_shop), NAPI.Util.ToJson(product_forsell));
    }
    ...
    public static void CustomShopRespone(Player player, int selected, string selectedname, int purchasedAmount)
    {
        ...
        if (entrace_data[PBID].Products[selected].name == selectedname)
        {
            ...
            {
                Main.SendCustomChatMessasge(player, "[~g~Mülk~w~] Bir şeyler satın aldınız.");
                return;
            }
            if (entrace_data[PBID].Products[selected].amount < purchasedAmount)
            {
                Main.SendCustomChatMessasge(player, "[~g~Mülk~w~] Bir şeyler sattınız.");
                return;
            }
    ...
    public static void PressKeyK(Player Client)
    {
        if (Client.HasData("PB_index"))
        {
            ...
            {
                Main.DisplayErrorMessage(Client, "info", "~g~İş yerinizi ~w~açtınız.");
            ...
            {
                Main.DisplayErrorMessage(Client, "info", "~r~İş yerinizi ~w~kilitlediniz.");
    ...
    public void CMD_editentrance(Player Client, int entrace_id, string type, string data)
    {
        ...
        {
            Main.SendErrorMessage(Client, "Değer 1'den küçük veya " + MAX_ENTRACE + "'den büyük olamaz.");
            return;
        }

        if (type == "name")
        {
            InteractMenu.User_Input(Client, "input_entrace_name", "Giriş Adını Düzenle", entrace_data[entrace_id].name);
            Client.SetData("entrace_id", entrace_id);
        }
        else if (type == "owner")
        {
            Main.SendCustomChatMessasge(Client, entrace_id + " numaralı iş yerinin sahibi olarak " + data + " atanmıştır.");
        ...
        {
            Main.SendCustomChatMessasge(Client, entrace_id + " numaralı iş yerinin fiyatı " + data + " olarak ayarlandı.");
        ...
        {
            Main.SendCustomChatMessasge(Client, entrace_id + " numaralı iş yerinin dış mekânı, mevcut konumunuza ayarlandı.");
            ...
            entrace_data[entrace_id].label = NAPI.TextLabel.CreateTextLabel("~y~" + entrace_data[entrace_id].name + "~w~~n~~n~" + entrace_data[entrace_id].description, entrace_data[entrace_id].exterior, 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[entrace_id].exterior_dimension);

            SaveEntrace(entrace_id);
        }
        else if (type == "interior")
        {
            Main.SendCustomChatMessasge(Client, entrace_id + " numaralı iş yerinin iç mekânı, mevcut konumunuza ayarlandı.");
            ...
            entrace_data[entrace_id].label_interior = NAPI.TextLabel.CreateTextLabel("- ~y~Çıkış~w~ -", entrace_data[entrace_id].interior, 5.0f, 0.6f, 4, new Color(0, 122, 255, 150), false, entrace_data[entrace_id].exterior_dimension);

            SaveEntrace(entrace_id);
        }
        else if (type == "delete")
        {
            Main.SendCustomChatMessasge(Client, entrace_id + " numaralı iş yeri silindi.");
    ...
    public static void OnInputResponse(Player Client, String response, String inputtext)
    {
        if (response == "input_entrace_name")
        {
            ...
            Main.SendCustomChatMessasge(Client, "İş yerinin adı " + Client.GetData<dynamic>("entrace_id") + " olarak ~b~" + inputtext + "~w~ ayarlandı.");
            SaveEntrace(Client.GetData<dynamic>("entrace_id"));
            entrace_data[Client.GetData<dynamic>("entrace_id")].label_interior.Text = "- Çıkış -";
            entrace_data[Client.GetData<dynamic>("entrace_id")].label.Text = "" + entrace_data[Client.GetData<dynamic>("entrace_id")].name + "~n~~n~Çıkış için ~y~Y~w~ kullanın.";
