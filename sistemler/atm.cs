...
class ATMSystem : Script
{
    ...
    public static void ATMRob(Player Client)
    {
        foreach (var atm in atms)
        {
            if (Main.IsInRangeOfPoint(Client.Position, atm.position, 3.0f))
            {
                if (atmpljacka == true)
                {
                    Main.DisplayErrorMessage(Client, "info", "ATM zaten soyulmuş.");
                    return;
                }
                    ...
                    NAPI.Task.Run(() =>
                    {
                        if (NAPI.Player.IsPlayerConnected(Client))
                        {
                            if (Main.IsInRangeOfPoint(Client.Position, atm.position, 3.0f) && (Inventory.GetPlayerItemFromInventory(Client, 77) > 0))
                            {
                                ...
                                Main.DisplayErrorMessage(Client, "success", "ATM'den $" + robmoney + " çaldınız.");
                                return;
                            }
                            else
                            {
                                Main.DisplayErrorMessage(Client, "info", "ATM'den çok uzağa gittiniz.");
                                Client.StopAnimation();
                                BasicSync.DetachObject(Client);
                                return;
                            }
    ...
    public static void BankWithdrawMoney(Player Client, int amount)
    {
        ...
        {
            Main.DisplayErrorMessage(Client, "error", "Miktar 1 ile 1.000.000 arasında olmalıdır.");
            return;
        }
        if (Main.GetPlayerBank(Client) < value)
        {
            Main.DisplayErrorMessage(Client, "error", "Yeterli miktarda bakiyeniz yok!");
            return;
        }
        double totalmoney = value * 0.01;
        ...
        Main.DisplayErrorMessage(Client, "info", value.ToString("N0") + " miktarında para çektiniz ve " + rounded + " vergi ödediniz.");
    ...
    public static void BankDepositMoney(Player Client, int amount)
    {
        ...
        {
            Main.DisplayErrorMessage(Client, "error", "Yeterli bakiyeniz yok.");
            return;
        }
        Main.GivePlayerMoney(Client, -value);
        Main.GivePlayerMoneyBank(Client, value);
        Main.DisplayErrorMessage(Client, "info", value.ToString("N0") + " miktarında para hesabınıza yatırıldı.");
    }
    ...
    public static void BankTransfareMoney(Player Client, string idOrName, int amount)
    {
        ...
        Main.DisplayErrorMessage(Client, "info", target.Name + " adlı kişiye " + value.ToString("N0") + " miktarında para gönderdiniz.");
        Main.DisplayErrorMessage(target, "info", Client.Name + " adlı kişi size " + value.ToString("N0") + " miktarında para gönderdi.");
    }
