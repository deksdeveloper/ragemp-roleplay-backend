...
class RobberyNPC : Script
{
    ...
    {
        robbery_npc.Add(new RobberyNPCEnum { id = MAX_ROBBERY_NPC, name = name, model = model, position = position, heading = heading, robbery_state = 0, players_aiming = 0, caixa_1 = caixa_1, caixa_2 = caixa_2, textlabel = API.Shared.CreateTextLabel("" + name + "", position + new Vector3(0, 0, 1.15), 5.0f, 0.3f, 4, new Color(0, 122, 255, 150), false, 0) });
    ...
    {
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(24.47675, -1347.312, 29.49702), 266.0985f, new Vector3(24.47675, -1347.312, 29.49702), new Vector3(24.47675, -1347.312, 29.49702));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-706.0944, -913.6349, 19.21561), 90.28362f, new Vector3(-706.0944, -913.6349, 19.21561), new Vector3(-706.0944, -913.6349, 19.21561));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-1819.664, 793.7935, 138.0857), 131.3555f, new Vector3(-1819.664, 793.7935, 138.0857), new Vector3(-1819.664, 793.7935, 138.0857));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-47.00174, -1758.323, 29.421), 64.01025f, new Vector3(-47.00174, -1758.323, 29.421), new Vector3(-47.00174, -1758.323, 29.421));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-1486.48, -377.7018, 40.16343), 136.0111f, new Vector3(-1486.48, -377.7018, 40.16343), new Vector3(-1486.48, -377.7018, 40.16343));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(372.734, 327.3333, 103.5664), 248.5667f, new Vector3(372.734, 327.3333, 103.5664), new Vector3(372.734, 327.3333, 103.5664));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(1164.873, -323.3881, 69.20515), 98.66148f, new Vector3(1164.873, -323.3881, 69.20515), new Vector3(1164.873, -323.3881, 69.20515));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-3242.331, 999.9945, 12.83071), 357.289f, new Vector3(-3242.331, 999.9945, 12.83071), new Vector3(-3242.331, 999.9945, 12.83071));
        CreateRobberyNPC("Satıcı", "g_m_y_lost_03", new Vector3(-3173.577, 1088.357, 20.83873), 244.3044f, new Vector3(-3173.577, 1088.357, 20.83873), new Vector3(-3173.577, 1088.357, 20.83873));
    
        
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(1727.791, 6415.229, 35.03723), 235.9682f, new Vector3(1727.791, 6415.229, 35.03723), new Vector3(1727.791, 6415.229, 35.03723));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(2678.062, 3279.332, 55.24113), 328.373f, new Vector3(2678.062, 3279.332, 55.24113), new Vector3(2678.062, 3279.332, 55.24113));
    
        
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(1134.17, -982.61, 46.41), -82.483f, new Vector3(1134.17, -982.61, 46.41), new Vector3(1134.17, -982.61, 46.41));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-1221.99, -908.40, 12.32), 28.493f, new Vector3(-1221.99, -908.40, 12.32), new Vector3(-1221.99, -908.40, 12.32));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(1959.99, 3740.05, 32.34), -64.493f, new Vector3(1959.99, 3740.05, 32.34), new Vector3(1959.99, 3740.05, 32.34));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(1697.70, 4923.12, 42.06), -37.493f, new Vector3(1697.70, 4923.12, 42.06), new Vector3(1697.70, 4923.12, 42.06));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(1391.90, 3606.14, 34.98), -164.493f, new Vector3(1391.90, 3606.14, 34.98), new Vector3(1391.90, 3606.14, 34.98));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-2966.34, 390.95,15.04), 86.493f, new Vector3(-2966.34, 390.95,15.04), new Vector3(-2966.34, 390.95,15.04));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(-3038.89, 584.48, 7.90), 14.253f, new Vector3(-3038.89, 584.48, 7.90), new Vector3(-3038.89, 584.48, 7.90));
        CreateRobberyNPC("Satıcı", "ig_popov", new Vector3(2557.23, 380.74, 108.62), -8.253f, new Vector3(2557.23, 380.74, 108.62), new Vector3(2557.23, 380.74, 108.62));
            ...
            foreach (var robbery in robbery_npc)
            {
                ...
                if (robbery.robbery_state == 1)
                {
                    ...
                    if (robbery.time_remaining == 0)
                    {
                        ...
                        robbery.textlabel.Text = "[ ~b~Satıcı~w~ ]~n~~n~~y~((~w~ " + robbery.name + " ~y~))";
                    }
                    ...
                    if (count == 0)
                    {
                        ...
                        robbery.textlabel.Text = "[ ~b~DİKKAT~w~ ]~n~~n~~y~((~w~ " + robbery.name + " ~y~))";
                    }
                    else
                    {
                        UpdateRobberyState(index, 1);
                        robbery.players_aiming = 1;
                        robbery.textlabel.Text = "~r~** Mağaza **~w~~n~~n~[ ~b~DİKKAT~w~ ]~n~~n~~y~((~w~ " + robbery.name + " ~y~))";
                    }
    ...
    public static void keyPressE(Player Client)
    {
        foreach (var robbery in robbery_npc)
        {
            if (Main.IsInRangeOfPoint(Client.Position, robbery.position, 3.0f) && robbery.robbery_state == 1)
            {
                ...
                {
                    Main.SendCustomChatMessasge(Client, "~r~ Sürekli olarak silahınızı satıcıya doğrultmalısınız!");
                    return;
                }
                ...
                player_robbery_timer[Main.getIdFromClient(Client)] = TimerEx.SetTimer(() =>
                {
                    if (robbery.players_aiming == 0)
                    {
                        Main.SendCustomChatMessasge(Client, "~r~ Satıcı ellerini kaldırmadı!");
                        Client.StopAnimation();
                        Client.TriggerEvent("freezeEx", false);
                    ...
                    else
                    {
                        if (robbery.cash_amount > 0)
                        {
                            ...
                            Client.TriggerEvent("createNewHeadNotificationAdvanced", "~g~Soygun başarılı. ~b~$" + robbery.cash_amount + "~g~!");
                        ...
                        Client.StopAnimation();
                        InteractMenu_New.SendNotificationInfo(Client, "Parayı aldınız!");
                        ...
                        robbery.textlabel.Text = "[ ~g~24-7 Satıcı~w~ ]~n~~n~~y~((~w~ Korkmuş ~y~))";
                    }
                ...
                if (robbery_npc[index].robbery_state == 0)
                {
                    if (DateTime.Now < robbery_npc[index].time_vulnerable)
                    {
                        if (!Client.HasData("temp_message"))
                        {
                            ...
                            var res = String.Format($"{time_left.Minutes} dakika ve {time_left.Seconds} saniye");
                            Main.SendErrorMessage(Client, "Bu mağaza zaten soyulmuş! ~c~~n~~n~ Mağaza " + res + " sonra tekrar soyulabilir.");
                        }
                    ...
                    {
                        Main.DisplaySubtitle(Client, "Ateşli silahınız yok.");
                        return;
                    }
                    ...
                    Main.SendCustomChatMessasge(Client, Main.EMBED_GREY + "Siz satıcıya nişan alırken arkadaşınız parayı alıyor.");
                ...
                if (robbery_npc[indexid].robbery_state == 1 && robbery_npc[indexid].robbery_reported == false)
                {
                    foreach (Player target in NAPI.Pools.GetAllPlayers())
                    {
                        if (target.GetData<dynamic>("status") == true && AccountManage.GetPlayerGroup(target) == 1)
                        {
                            ...
                            Main.SendCustomChatMessasge(target, Main.EMBED_BLUE + "[CENTRAL]" + Main.EMBED_COP_Message + "Tüm birimlerin dikkatine, bir mağaza soygunu gerçekleşiyor!");
    ...
    public static void Set_Robbery_Blip(int index)
    {
        foreach (Player target in API.Shared.GetAllPlayers())
        {
            if (target.GetData<dynamic>("status") == true && FactionManage.GetPlayerGroupID(target) == 1)
            {
                ...
                {
                    target.TriggerEvent("blip_create_ext", "Problem_" + index, robbery_npc[index].position, 49, 2f, 8);
        ...
        try
        {
            ...
            foreach (Player target in API.Shared.GetAllPlayers())
            {
                if (target.GetData<dynamic>("status") == true)
                {
                    ...
                    {
                        target.TriggerEvent("blip_remove", "Problem_" + index);
