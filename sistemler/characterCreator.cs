...
namespace CharCreator
{
    ...
    public class CharCreator : Script
    {
        ...
        public CharCreator()
        {
            ...
            shape_mix_list.Add("Anne - ~b~I~w~IIIIIIII ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~II~w~IIIIIII ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~III~w~IIIIII ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~IIII~w~IIIII ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~IIIII~w~IIII ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~IIIIII~w~III ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~IIIIIII~w~II ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~IIIIIIII~w~I ~w~- ~y~Baba");
            shape_mix_list.Add("Anne - ~b~IIIIIIIII ~w~- ~y~Baba");
        ...
        public static void SaveCharacterFromCreation(Player Client, string name,sbyte age)
        {
            ...
            if (age < 15 || age > 80)
            {
                Client.SendNotification("Hata!~n~Yaşınız en az 15, en fazla 80 olabilir.");
                return;
            }
        ...
        public static void Proccess_Character(Player Client)
        {
            ...
            {
                Client.SendNotification("Hata!~n~İsim ve soyisim 6 ile 24 harf arasında olmalıdır.");
                return;
            }
            ...
            {
                Client.SendNotification("Hata!~n~İsim ve soyisminizde boşluk olmamalıdır.");
                return;
            }
            ...
            if (Utils.isRoleplayName(inputtext) == true)
            {
                using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
                {
                    ...
                    using (MySqlDataAdapter da = new MySqlDataAdapter(query))
                    {
                        ...
                        {
                            Client.SendNotification("Hata!~n~Bu isim zaten kullanılıyor.");
                            //InteractMenu.User_Input(Client, "interact_character_newname", "Karakter İsmi (Örnek: Thiago_Gomes)", "Thiago_Gomes");
                        }
                    }
                    Mainpipeline.Close();
                }
            ...
            {
                Client.SendNotification("Hata!~n~İsminiz şu formatta olmalıdır: ~y~Deks_Michealson");
                //InteractMenu.User_Input(Client, "interact_character_newname", "Karakter İsmi (Örnek: Thiago_Gomes)", "Thiago_Gomes");
            }
        }
