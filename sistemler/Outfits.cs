...
class Outfits : Script
{
    ...
    [Command("setoutfit", "/setoutfit [id/isim] [id(0 / 1482)]")]
    public void command_settraje(Player Client, string idOrName, int value)
    {
        ...
        if (target != null)
        {
            ...
            GameLog.ELog(Client, GameLog.MyEnum.Admin, " /setoutfit " + AccountManage.GetCharacterName(target) + " " + value.ToString());

            if (Client != target) Main.SendCustomChatMessasge(Client, " ~y~" + AccountManage.GetCharacterName(target) + " dış görünüm " + value + " olarak ayarlandı.");
            else Main.SendInfoMessage(Client, "~y~" + AccountManage.GetCharacterName(target) + " size dış görünüm olarak: " + value + " ayarladı.");
    ...
    public static void SetMaleOutfit(Player Client, int ID)
    {
        ...
        {
            Main.SendCustomChatMessasge(Client,"Geçersiz ID: 0 - " + (MaleOutfits.Count - 1) + ".");
            return;
        }
    ...
    public static void SetFemaleOutfit(Player Client, int ID)
    {
        if (ID < 0 || ID >= FemaleOutfits.Count)
        {
            ...
            Main.SendCustomChatMessasge(Client,"Geçersiz ID: 0 - " + (FemaleOutfits.Count - 1) + ".");
            return;
        }
    ...
    public static void SetUnisexOutfit(Player Client, int ID, bool save = false)
    {
        ...
        switch ((PedHash)Client.Model)
        {
            ...
            default:
                Main.SendCustomChatMessasge(Client, "Sizde ayarlanmış bir kıyafet hali hazırda var!");
                break;
        }
    }
