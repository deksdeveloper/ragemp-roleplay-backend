...
class AntiCheat : Script
{
    ...
    public static void AntiCheatLog(Player player, string log)
    {
        if (AccountManage.GetPlayerConnected(player))
        {
            foreach (var item in NAPI.Pools.GetAllPlayers())
            {
                ...
                {
                    Main.SendCustomChatMessasge(item, "~r~[AntiCheat] ~c~" + player.Name + "" + "~w~ adlı oyuncunun '" + log + "' kullandığı tespit edildi.");

                    GameLog.ELog(player, GameLog.MyEnum.Anti_Cheat, player.Name + "(" + player.SocialClubName + ")" + " " + log + " hilesini kullanıyor.");
                }
            }
        }
    }

    public static void AntiAmmoCheat(Player player, int mermi, int olmasigereken)
    {
        if (AccountManage.GetPlayerConnected(player))
        {
            foreach (var item in NAPI.Pools.GetAllPlayers())
            {
                if (AccountManage.GetPlayerAdmin(item) >= 6)
                {
                    Main.SendCustomChatMessasge(item, "~r~[AntiCheat] ~c~" + player.Name + "" + "~w~ adlı oyuncunun 'Mermi Hilesi' kullandığı tespit edildi.");
                    Main.SendCustomChatMessasge(item, $"~r~[AntiCheat] ~c~ Detaylar: ~w~ Mermi miktarı: {mermi} - Olması gereken mermi: {olmasigereken}");
                    GameLog.ELog(player, GameLog.MyEnum.Anti_Cheat, player.Name + "(" + player.SocialClubName + ")" + " 'Mermi Hilesi' hilesini kullanıyor.");
                }
            }
        }
    }
