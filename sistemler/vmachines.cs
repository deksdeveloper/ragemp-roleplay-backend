    ...
    public static void keypressE(Player player)
    {
        ...
        foreach (var v in sprunkmasine)
        {
            if (player.Position.DistanceTo(v) < 1)
            {
                ...
                {
                    player.SendNotification("Yeterli paranýz yok.");
                    return;
                }
                ...
                NAPI.Task.Run(() =>
                {
                    if (NAPI.Player.IsPlayerConnected(player))
                    {
                        ...
                        player.SendNotification("$35 ödeyerek ~g~Spunk~w~ satýn aldýnýz!");
            ...
            if (player.Position.DistanceTo(v) < 1)
            {
                ...
                NAPI.Task.Run(() =>
                {
                    if (NAPI.Player.IsPlayerConnected(player))
                    {
                        ...
                        player.SendNotification("$35 ödeyerek ~r~Ecolog~w~ satýn aldýnýz!");
                        player.SetData<dynamic>("kupujecolu", false);
                    }
                }, delayTime: 5000);
            }
            ...
            if (player.Position.DistanceTo(v) < 1)
            {
                ...
                NAPI.Task.Run(() =>
                {
                    if (NAPI.Player.IsPlayerConnected(player))
                    {
                        ...
                        player.SendNotification("$10 ödeyerek ~o~Coffee ToGo~w~ satýn aldýnýz!");
                        player.SetData<dynamic>("kupujecolu", false);
                    }
                }, delayTime: 5000);
            }
            ...
            if (player.Position.DistanceTo(v) < 1)
            {
                ...
                NAPI.Task.Run(() =>
                {
                    if (NAPI.Player.IsPlayerConnected(player))
                    {
                        ...
                        player.SendNotification("$35 ödeyerek ~b~Raine~w~ satýn aldýnýz.");
                        player.SetData<dynamic>("kupujecolu", false);
                    }
                }, delayTime: 5000);
            }
