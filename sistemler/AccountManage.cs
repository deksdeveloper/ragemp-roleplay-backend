...
class AccountManage : Script
{
        ...
        if (GetPlayerConnected(Client))
        {
            ...
            using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
            {
                try
                {
                    ...
                    query = query + ", `radio_freq` = @radio_freq, `timestamp` = @timestamp, `fontsize` = @fontsize, `pagesize` = @pagesize, `BalikLevel` = @BalikLevel, `BalikEXP` = @BalikEXP";
                    ...
                    myCommand.Parameters.AddWithValue("@character_cellphone", TelefonSistem.GetPlayerNumber(Client));

                    myCommand.Parameters.AddWithValue("@inside_house", Client.GetData<int>("OyuncuEv"));

                    myCommand.Parameters.AddWithValue("@timestamp", Client.GetData<bool>("timeStamp"));
                    myCommand.Parameters.AddWithValue("@fontsize", Client.GetData<double>("fontSize"));
                    myCommand.Parameters.AddWithValue("@pagesize", Client.GetData<int>("pageSize"));
                    myCommand.Parameters.AddWithValue("@BalikLevel", Client.GetData<int>("BalikLevel"));
                    myCommand.Parameters.AddWithValue("@BalikEXP", Client.GetData<int>("BalikEXP"));
    ...
    public static void SaveDiscordInfo(Player Client)
    {
        ...
        if (GetPlayerConnected(Client))
        {
            ...
            using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
            {
                ...
                {
                    Mainpipeline.Open();
                    string query = "UPDATE users SET `DiscordUsername` = @DiscordUsername, `DiscordID` = @DiscordID";
                    query = query + " WHERE id = '" + NAPI.Data.GetEntityData(Client, "player_sqlid") + "'";
                    MySqlCommand myCommand = new MySqlCommand(query, Mainpipeline);

                    myCommand.Parameters.AddWithValue("@DiscordID", Client.GetData<string>("DiscordID"));
                    myCommand.Parameters.AddWithValue("@DiscordUsername", Client.GetData<string>("DiscordUsername"));
                ...
                {
                    NAPI.Util.ConsoleOutput("[EXCEPTION SaveDiscordInfo] " + ex.Message);
                }
            }
        ...
        using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
        {
            ...
            using (MySqlDataReader reader = query.ExecuteReader())
            {
                ...
                while (reader.Read())
                {
                    ...
                    Client.SetData("BalikTutuyor", false);
                    Client.SetData("SecilenYem", Balikcilik.YEM_YOK);
                    ...
                    Client.SetData<int>("OyuncuEv", reader.GetInt32("inside_house"));
                    ...
                    Client.SetData<dynamic>("BalikLevel", reader.GetInt32("BalikLevel"));
                    Client.SetData<dynamic>("BalikEXP", reader.GetInt32("BalikEXP"));

                    Client.SetData("timeStamp", reader.GetBoolean("timestamp"));
                    Client.SetData("toggleChat", false);
                    Client.SetData("fontSize", reader.GetDouble("fontsize"));
                    Client.SetData("pageSize", reader.GetInt32("pagesize"));
                    Main.loadChatSettings(Client);
                    // Default Player data
    ...
    public static void ColshapeFix(Player Client)
    {
        Vector3 oyuncupos = Client.Position;

        NAPI.Task.Run(() =>
        {
            NAPI.Entity.SetEntityPosition(Client, new Vector3(Client.Position.X + 100, Client.Position.Y, Client.Position.Z));

        }, delayTime: 1000);
        NAPI.Task.Run(() =>
        {
            NAPI.Entity.SetEntityPosition(Client, oyuncupos);
    ...
    public static void OnLoadCharacterData(Player Client)
    {
        ...
        if (GetPlayerAdmin(Client) >= 1)
        {
            ...
            foreach (var i in players)
            {
                ...
                {
                    Main.SendCustomChatMessasge(i, "~r~" + adminCommands.GetPlayerAdminRank(Client) + " ~b~" + GetCharacterName(Client) + "~w~ sunucuya giriş yaptı.");
                }
            }
        }
        ...
        NAPI.Task.Run(() =>
        {
            ...
            if (TelefonSistem.GetPlayerNumber(Client) != 0)
            {
                ...
                Client.TriggerEvent("setPhoneNumber", TelefonSistem.GetPlayerNumber(Client));
            }
            ...
            Main.enableChat(Client);
            Main.SendSuccessMessage(Client, "Sunucuya giriş yaptınız, veriler başarıyla yüklendi!");
            Client.TriggerEvent("GuvenliBolge", false);
            ColshapeFix(Client);
            NAPI.Player.SetPlayerHealth(Client, Client.GetData<dynamic>("character_health"));
        }, delayTime: 8500);
        Client.TriggerEvent("setResistStage", 0);

        if (Client.GetData<int>("OyuncuEv") != -1)
        {
            uint houseDimension = (uint)(Client.GetData<int>("OyuncuEv") + 1000);
            Client.Dimension = houseDimension;
        }
    }
    ...
    public static async Task CreateAccount(Player Client, string password, string email, string invitedrefferal)
    {
        using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
        {
            ...
            using (MySqlDataAdapter da = new MySqlDataAdapter(query))
            {
                ...
                {
                    Notify.Send(Client, "error", "Kayıt sırasında bir sorun oluştu.", 5000);
                }
            }
            Mainpipeline.Close();
        }
    ...
    public static async Task LoadAccount(Player Client, string user)
    {
        ...
        {
            Mainpipeline.Open();
            MySqlCommand query = new MySqlCommand("SELECT isban,betaAcess,id,email,socialclubname,banreason,bannedby,logged,refferal,DiscordUsername,DiscordID FROM `users` WHERE `socialclubname` = '" + Client.SocialClubName + "' ", Mainpipeline);
            using (MySqlDataReader reader = query.ExecuteReader())
            {
                string data2txt = string.Empty;
                string datatxt = string.Empty;
                ...
                while (reader.Read())
                {
                    ...
                    if (reader.GetInt32("isban") == 1)
                    {
                        string account = reader.GetString("socialclubname");
                        string reason = reader.GetString("banreason");
                        string admin = reader.GetString("bannedby");

                        Notify.Send(Client, "error", $"[BANLANDINIZ] Hesap: {account}\nSebep: {reason}\nYetkili: {admin}", 10000);
                        Client.Kick();
                        return;
                    }
                    ...
                    Client.SetData("DiscordCode", "");
                    
                    Client.SetData("DiscordUsername", reader.GetString("DiscordUsername"));
                    Client.SetData("DiscordID", reader.GetString("DiscordID"));
    ...
    public static async Task CharacterSelection(Player Client)
    {
        ...
        using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
        {
            ...
            using (MySqlDataReader reader = query.ExecuteReader())
            {
                ...
                NAPI.Entity.SetEntityPosition(Client, new Vector3(393.2795, -355.5835, 48.027664));
                Client.Rotation = new Vector3(0, 0, -71.62504);
                Main.disableChat(Client);
                NAPI.Task.Run(() =>
                {
                    NAPI.Entity.SetEntityPosition(Client, new Vector3(393.2795, -355.5835, 48.027664));
                    Client.Rotation = new Vector3(0, 0, -71.62504);
    ...
    [Command("testpos")]
    public static void TestPos(Player player)
    {
        NAPI.Entity.SetEntityPosition(player, new Vector3(393.2795, -355.5835, 48.027664));
        player.Rotation = new Vector3(0, 0, -71.62504);
    }
    ...
    public static async Task SelectCharacter(Player Client, int character_id)
    {
        ...
        {
            Notify.Send(Client, "error", "Bir karakter seçin.", 5000);
            return;
        }
    ...
    {
        if (Main.g_mysql_get_characters_created(Client) >= 3 && Main.g_mysql_get_VIP_status(Client) == true)
        {
            Notify.Send(Client, "error", "Maksimum 3 adet karakter oluşturabilirsiniz.", 5000);
            return;
        }
        if (Main.g_mysql_get_characters_created(Client) >= 2 && Main.g_mysql_get_VIP_status(Client) == false)
        {
            Notify.Send(Client, "error", "Maksimum 2 adet karakter oluşturabilirsiniz!", 5000);
            return;
        }
        else
        {
                ...
                while (reader.Read())
                {
                    ...
                    foreach (var item in NAPI.Pools.GetAllPlayers())
                    {
                        if (AccountManage.GetPlayerConnected(item))
                        {
                            ...
                            {
                                Main.SendMessageToPlayer(item, "[~g~Referans~w~] Birisi sizin referans kodunuzla kayıt olduğu için ~g~$" + Refferalerguymoney + "~w~ ve +2 günlük VIP kazandınız!");
