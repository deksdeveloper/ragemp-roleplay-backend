...
class houserobbery : Script
{
    ...
    public static void hrobRob(Player Client)
    {
        foreach (var hrob in robhouses)
        {
            if (Main.IsInRangeOfPoint(Client.Position, hrob.Position, 3.0f))
            {
                ...
                {
                    Main.DisplayErrorMessage(Client, "info", "Bu ev zaten soyulmuþ.");
                    return;
                }
                ...
                if (Inventory.GetPlayerItemFromInventory(Client, 20) > 0)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        if (NAPI.Player.IsPlayerConnected(Client))
                        {
                            ...
                            {
                                Main.DisplayErrorMessage(Client, "info", "Kapýdan çok uzaklaþtýnýz.");
                                return;
                            }
                        }
                    }, delayTime: 30000);
    ...
    public static void SellItems(Player Client)
    {
        if (Inventory.GetPlayerItemFromInventory(Client, 78) > 0)
        {
            ...
            Main.DisplayErrorMessage(Client, "success", "Eþyalarý sattýnýz ve $" + totalnakit * 1000 + " kazandýnýz.");
        }
    }
                    ...
                    if (i == 5)
                    {
                        if (Inventory.GetPlayerItemFromInventory(client, 28) > 0)
                        {
                            ...
                            if (randomksd == 2)
                            {
                                ...
                                Main.DisplayErrorMessage(client, "info", "Maymuncuk kýrýldý ve bir parçasý kilitte kaldý...");
                                return;
                            }
                            NAPI.Task.Run(() =>
                            {
                        ...
                        {
                            Main.DisplayErrorMessage(client, "info", "Maymuncuðunuz yok");
                            return;
                        }
                    }
                    break;
