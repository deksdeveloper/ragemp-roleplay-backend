...
class WerehouseManage : Script
{
    ...
    public static void WerehouseManageInit()
    {
        ...
        using (MySqlConnection Mainpipeline = new MySqlConnection(Main.myConnectionString))
        {
            ...
            using (MySqlDataReader reader = query.ExecuteReader())
            {
                ...
                while (reader.Read())
                {
                    ...
                    WereHouseData[index].exteriorLabel = NAPI.TextLabel.CreateTextLabel(" ", new Vector3(WereHouseData[index].exterior_x, WereHouseData[index].exterior_y, WereHouseData[index].exterior_z + 0.6), 14, 0.600f, 0, new Color(0, 122, 255, 150));
                    WereHouseData[index].exteriorMarker = NAPI.Marker.CreateMarker(30, new Vector3(WereHouseData[index].exterior_x, WereHouseData[index].exterior_y, WereHouseData[index].exterior_z - 0.3), new Vector3(), new Vector3(), 0.8f, new Color(0, 122, 255, 150));
                    ...
                    WereHouseData[index].menuArea.OnEntityEnterColShape += (s, ent) =>
                    {
                        ...
                        if ((Client = NAPI.Player.GetPlayerFromHandle(ent)) != null)
                        {
                            ...
                            Main.DisplaySubtitle(Client, "Menüye erişmek için <C>[ ~b~Y~w~ ]</C> tuşunu kullanın", 3);
                        }
                    };


                    WereHouseData[index].menuLabel = API.Shared.CreateTextLabel("~g~~y~ Depo~g~", new Vector3(WereHouseData[index].menu_x, WereHouseData[index].menu_y, WereHouseData[index].menu_z - 0.2f), 8.0f, 0.8f, 4, new Color(0, 122, 255, 150));
        ...
        API.Shared.CreateTextLabel("~g~-~b~ Vracanje vozila~g~ -~w~~n~~n~Koristite ~y~Y~w~ da vratite vozilo", new Vector3(463.67, -1017.206, 27.88313 - 0.2f), 8.0f, 0.8f, 4, new Color(0, 122, 255, 150));
    ...
    public static void UpdateWerehousePickup(int index)
    {
        ...
        {
            WereHouseData[index].exteriorLabel = NAPI.TextLabel.CreateTextLabel("Tanımsız HQ " + index + "", new Vector3(WereHouseData[index].exterior_x, WereHouseData[index].exterior_y, WereHouseData[index].exterior_z + 0.6), 14, 0.600f, 0, new Color(0, 122, 255, 150));
        }
        else WereHouseData[index].exteriorLabel = NAPI.TextLabel.CreateTextLabel("~w~[ ~c~" + WereHouseData[index].name + "~w~ ]", new Vector3(WereHouseData[index].exterior_x, WereHouseData[index].exterior_y, WereHouseData[index].exterior_z + 0.6), 14, 0.600f, 0, new Color(0, 122, 255, 150));
        WereHouseData[index].exteriorMarker = NAPI.Marker.CreateMarker(0, new Vector3(WereHouseData[index].exterior_x, WereHouseData[index].exterior_y, WereHouseData[index].exterior_z - 0.3), new Vector3(), new Vector3(), 0.8f, new Color(0, 122, 255, 150));
    }
    ...
    public static void ShowWarehouseMenu(Player Client)
    {
        ...
        foreach (var werehouse in WereHouseData)
        {
            ...
            if (Main.IsInRangeOfPoint(Client.Position, new Vector3(werehouse.menu_x, werehouse.menu_y, werehouse.menu_z), 1))// && NAPI.Entity.GetEntityDimension(Client) == count + 500
            {
                ...
                menu_item_list.Add(new { Type = 1, Name = "Para", Description = "" });
                //menu_item_list.Add(new { Type = 1, Name = "Silah Al / Sipariş Et", Description = "" });
                //menu_item_list.Add(new { Type = 1, Name = "Silah Sakla / Ekipman", Description = "" });

                InteractMenu.CreateMenu(Client, "WEREHOSE_MENU", "Organizasyon", "~b~Kasa", false, NAPI.Util.ToJson(menu_item_list), false);
    ...
    public static void SelectMenuResponse(Player Client, String callbackId, int selectedIndex, String objectName, String valueList)
    {
                ...
                case 0:
                    {
                        ...
                        menu_item_list.Add(new { Type = 1, Name = "Para yatır", Description = "Parayı kasaya yatırma." });
                        menu_item_list.Add(new { Type = 1, Name = "Para al", Description = "Kasadan para çekme." });

                        InteractMenu.CreateMenu(Client, "WEREHOSE_MENU_SAFE", "Organizasyon", "~b~Kasa ~g~($" + WereHouseData[index].safe.ToString("N0") + ")", false, NAPI.Util.ToJson(menu_item_list), false);
                        break;
                    }
                case 1:
                    {
                        ...
                        for (int i = 0; i < 20; i++)
                        {
                            ...
                            {
                                menu_item_list.Add(new { Type = 1, Name = i + ". " + WereHouseData[index].gun[i] + " (İzin: ~c~" + WereHouseData[index].gun_perm[i] + " +~w~)", Description = "", RightLabel = WereHouseData[index].gun_units[i] + " adet" });
                            }
                            else
                            {
                                menu_item_list.Add(new { Type = 1, Name = i + ". Boş", Description = "" });
                            }
                        }
                        InteractMenu.CreateMenu(Client, "WEREHOSE_MENU_ORDER_EQUIPMENT", "Depo", "~b~Seçenekler:", false, NAPI.Util.ToJson(menu_item_list), false);
                        break;
                    }
                case 2:
                    {
                        ...
                        for (int i = 0; i < 20; i++)
                        {
                            if (WereHouseData[index].safe_gun[i] != "Bilinmiyor")
                            {
                                menu_item_list.Add(new { Type = 1, Name = i + ". ~g~" + WereHouseData[index].safe_gun[i] + "~w~", Description = "" });
                            }
                            else
                            {
                                menu_item_list.Add(new { Type = 1, Name = i + ". Kaydet", Description = "" });
                            }
                        }
                        InteractMenu.CreateMenu(Client, "WEREHOSE_MENU_SAFE_EQUIPMENT", "Depo", "~b~Seçenekler:", false, NAPI.Util.ToJson(menu_item_list), false);
                        break;
                    }
        ...
        else if (callbackId == "WEREHOSE_MENU_SAFE")
        {
            if (selectedIndex == 0)
            {
                InteractMenu.User_Input(Client, "input_werehouse_deposit", "Para yatırma", "0", "", "miktar");
                InteractMenu.CloseDynamicMenu(Client);
            }
            else
            {
                InteractMenu.User_Input(Client, "input_werehouse_withdraw", "Para çekme", "0", "", "miktar");
                InteractMenu.CloseDynamicMenu(Client);
            }
        }
        else if (callbackId == "WEREHOSE_MENU_ORDER_EQUIPMENT")
        {
            ...
            if (WereHouseData[index].gun_units[slotid] >= 1)
            {
                if (AccountManage.GetPlayerRank(Client) >= WereHouseData[index].gun_perm[slotid])
                {
                    ...
                    {
                        Main.SendCustomChatMessasge(Client, "Silahı ~g~" + WereHouseData[index].gun[slotid] + "~w~ depodan aldınız.");

                        WereHouseData[index].gun[slotid] = "Bilinmiyor";
                        WereHouseData[index].gun_units[slotid] = 0;
                        WereHouseData[index].gun_perm[slotid] = 0;
                    }
                    else Main.SendCustomChatMessasge(Client, "Depoda ~g~" + WereHouseData[index].gun[slotid] + "~w~ var.");
                    SaveWerehouse(index);
                    ShowWarehouseMenu(Client);
                }
                else
                {
                    Main.SendErrorMessage(Client, "Yetkiniz yok, ~c~" + FactionManage.faction_data[gangid].faction_rank[WereHouseData[index].gun_perm[slotid]] + " (" + WereHouseData[index].gun_perm[slotid] + ")~w~ ya da daha yüksek rütbeye sahip olanlar erişebilir.");
                    ShowWarehouseMenu(Client);
                }
            }
            else
            {
                ...
                List<dynamic> menu_item_list = new List<dynamic>();
                menu_item_list.Add(new { Type = 6, Name = "Silah", Description = "Silah seçimi.", List = model_list, StartIndex = 0 });
                menu_item_list.Add(new { Type = 6, Name = "Miktar", Description = "Miktar yönetimi.", List = list, StartIndex = 0 });
                menu_item_list.Add(new { Type = 6, Name = "İzinler", Description = "Üye izinleri.", List = permi_list, StartIndex = 0 });
                menu_item_list.Add(new { Type = 1, Name = "Ekipman Siparişi", Description = "Ekipman siparişi verme." });

                InteractMenu.CreateMenu(Client, "WEREHOSE_MENU_ORDER_PURCHASE", "Organizasyon", "~b~Silah", false, NAPI.Util.ToJson(menu_item_list), false, BackgroundSprite: "shopui_title_gr_gunmod");
            }
        }
        else if (callbackId == "WEREHOSE_MENU_ORDER_PURCHASE")
        {
            ...
            if (WereHouseData[index].safe > preco)
            {
                ...
                Main.SendCustomChatMessasge(Client, "Şu kadar ~g~" + unidades + "~w~ adet ~b~" + modelo + "~w~ silahı ~g~$" + preco.ToString("N0") + "~w~ fiyatına sipariş ettiniz.");
                Main.SendCustomChatMessasge(Client, "Malzeme artık depoda.");
                //Main.SendCustomChatMessasge(Client, "Há encomendá chegará as ~g~15:00~w~ horas nos conteiner de All.");
            }
            else
            {
                Main.SendErrorMessage(Client, "Organizasyonun kasasında yeterli paranız yok!");
            }
        }
        else if (callbackId == "WEREHOSE_MENU_SAFE_EQUIPMENT")
        {
            ...
            if (WereHouseData[index].safe_gun[slotid] == "Bilinmiyor")
            {
                WeaponHash model = NAPI.Player.GetPlayerCurrentWeapon(Client);
                if (model != NAPI.Util.WeaponNameToModel("Çıplak"))
                {
                    WereHouseData[index].safe_gun[slotid] = "" + model + "";
                    NAPI.Player.RemovePlayerWeapon(Client, model);
                    Main.SendCustomChatMessasge(Client, "Silahı ~g~" + model + "~w~ kasaya koydunuz.");
                    SaveWerehouse(index);
                    ShowWarehouseMenu(Client);
                }
                else
                {
                    ShowWarehouseMenu(Client);
                    Main.SendErrorMessage(Client, "Kasaya silah bırakabilmek için elinizde silah tutmalısınız.");
                }
            }
            else
            {
                ...
                NAPI.Player.SetPlayerCurrentWeaponAmmo(Client, 250);
                WereHouseData[index].safe_gun[slotid] = "Bilinmiyor";
                Main.SendCustomChatMessasge(Client, "Silahı ~g~" + model + "~w~ kasadan aldınız.");
                SaveWerehouse(index);
                ShowWarehouseMenu(Client);
            }
        }
    ...
    public static void UpdateOrderPrice(Player Client)
    {
        ...
        Main.DisplaySubtitle(Client, "Siparişin fiyatı ~c~" + weapon_order_list[current_weapon].order_model + "~w~: ~g~$~w~" + send_price + "", 60);
    ...
    public static void OnInputResponse(Player Client, String response, String inputtext)
    {
        if (response == "input_werehouse_deposit")
        {
            ...
            {
                Main.SendErrorMessage(Client, "Değer 1'den küçük ve 1.000.000'den büyük olamaz!");
                ShowWarehouseMenu(Client);
            }
            else
            {
                ...
                {
                    Main.SendCustomChatMessasge(Client, "Depoya ~g~$" + value.ToString("N0") + "~w~ bıraktınız!");
        ...
        else if (response == "input_werehouse_withdraw")
        {
            ...
            else
            {
                ...
                {
                    Main.SendCustomChatMessasge(Client, "Depodan ~g~$" + value.ToString("N0") + "~w~ aldınız.");
                    Main.GivePlayerMoney(Client, +value);
                    WereHouseData[gangid].safe -= value;
                    SaveWerehouse(gangid);
                    ShowWarehouseMenu(Client);
    ...
    [Command("hoodolustur", "/hoodolustur [Birlik ID] [çete/mafya]")]
    public void CMD_criararmazem(Player Client, int factionid, string type)
    {
        ...
        if (i == -1) { Main.SendErrorMessage(Client, "Yanlış giriş."); return; }
        if (Client.GetData<dynamic>("admin_duty") == 0 && AccountManage.GetPlayerAdmin(Client) <= 7)
        {
            Main.SendErrorMessage(Client, "Duty'niz açık değil, /ad komutunu kullanın.");
            return;
        }
        GameLog.ELog(Client, GameLog.MyEnum.Admin, " /createhq komutunu kullandı: " + factionid.ToString());
        ...
        //API.Shared.CreateTextLabel("~g~-~y~ Computador~g~ -~w~~n~~n~Koristiteione [ ~y~Y~w~ ] para acessar o computador", new Vector3(WereHouseData[i].menu_x, WereHouseData[i].menu_y, WereHouseData[i].menu_z - 0.2f), 8.0f, 0.8f, 4, new Color(0, 122, 255, 150));

        UpdateWerehousePickup(i);
        SaveWerehouse(i);
    }

    [Command("hoodduzenle", "/hoodduzenle [Birlik ID] [(name/price/outdoor/interior/odfe/menu/delete)] [Value(ako treba)] (da vidite prvi slobodan ID HQa /nexthq)")]
    public void CMD_editararmazem(Player Client, int index, string nome, string value = "null")
    {
        ...
        GameLog.ELog(Client, GameLog.MyEnum.Admin, " /edithq komutunu kullandı ");
        if (nome == "outdoor")
        {
            Main.SendCustomChatMessasge(Client, "Dış mekan ayarlarını " + index + " olarak değiştirdiniz.");
        ...
        if (nome == "menu")
        {
            Main.SendCustomChatMessasge(Client, "Menü ID'sini " + index + " olarak ayarladınız.");
        ...
        {
            Main.SendCustomChatMessasge(Client, "İç mekan ayarlarını " + index + " olarak değiştirdiniz.");
        ...
        else if (nome == "name")
        {
            ...
            Main.SendCustomChatMessasge(Client, "Depo ismini " + index + " " + value + " olarak değiştirdiniz.");
        ...
        {
            Main.SendCustomChatMessasge(Client, "HQ " + index + " fiyatını $" + Convert.ToInt32(value).ToString("N0") + " olarak değiştirdiniz.");
        ...
        {
            Main.SendCustomChatMessasge(Client, "HQ " + index + " güvenlik ücretini " + Convert.ToInt32(value).ToString("N0") + " olarak değiştirdiniz.");
        ...
        {
            Main.SendCustomChatMessasge(Client, "HQ ID: " + index + " silindi.");
    ...
    public void CMD_warehouse(Player Client)
    {
        ...
        Main.SendCustomChatMessasge(Client, "İlk boş HQ ID'si: ~y~" + GetArmazemFreeSlotID() + "~w~.");
    }
            ...
            if (Main.IsInRangeOfPoint(Client.Position, new Vector3(armazem.exterior_x, armazem.exterior_y, armazem.exterior_z), 3) || Main.IsInRangeOfPoint(Client.Position, new Vector3(armazem.interior_x, armazem.interior_y, armazem.interior_z), 3))
            {
                if (armazem.ownerid == AccountManage.GetPlayerGroup(Client))
                {
                    if (armazem.lockStatus == true)
                    {
                        ...
                        Main.SendCustomChatMessasge(Client, "~w~Depo ~g~açıldı ");
                    }
                    else
                    {
                        armazem.lockStatus = true;
                        Main.SendCustomChatMessasge(Client, "~w~Depo ~r~kilitlendi ");
                    }
                    return;
                }
            }
    ...
    public static void UI_GiveItem(Player Client, int slot, int type, int amount, int sqlid)
    {
        ...
        if (Inventory.player_inventory[playerid].amount[index] > 1)
        {
            ...
            {
                Main.SendErrorMessage(Client, "Yerin yok.");
                return;
            }
            ...
            Main.EmoteMessage(Client, "evin kasasından " + Inventory.itens_available[type].name + "aldı.");
    ...
    public static void ShowHQInventory(Player Client)
    {
        ...
        foreach (var entrace in WereHouseData)
        {
            if (Main.IsInRangeOfPoint(Client.Position, new Vector3(entrace.interior_x, entrace.interior_y, entrace.interior_z), 70.0f))
            {
                ...
                if (entrace.ownerid != AccountManage.GetPlayerGroup(Client))
                {
                    Main.SendErrorMessage(Client, "Organizasyon üyesi değilsiniz.");
                    return;
                }
    ...
    public static void UI_TakeItem(Player Client, int type, string inputtext)
    {
        try
        {
            ...
            if (WereHouseData[house_id].inventory_amount[index] > 1)
            {
                ...
                {
                    Main.SendErrorMessage(Client, "Yetersiz bakiye.");
                    return;
                }
                ...
                {
                    alvo.TriggerEvent("Send_ToChat", "", "<font color ='#C2A2DA'>* " + UsefullyRP.GetPlayerNameToTarget(Client, alvo) + " " + amount + "x " + Inventory.itens_available[type].name + " alıyor seferden.");
                }
            ...
            else if (WereHouseData[house_id].inventory_amount[index] == 1)
            {
                ...
                foreach (Player alvo in proxPlayers)
                {
                    alvo.TriggerEvent("Send_ToChat", "", "<font color ='#C2A2DA'>* " + UsefullyRP.GetPlayerNameToTarget(Client, alvo) + " " + Inventory.itens_available[type].name + " depodan alıyor.");
    ...
    public void HQ_SideInventorySplit(Player Client, int sqlid)
    {
        ...
        {
            Main.DisplayErrorMessage(Client, "info", "Ev için boş yeriniz yok.");
            return;
        }
